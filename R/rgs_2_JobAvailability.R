...
execIndicatorJobAvailability <- function(){
  ...
  # calculation
  mergedData = merge.data.frame(x=boundary@data, y=employmentSector, by.x = "sa2_main11", by.y = "sa2_main11", sort = FALSE)
  ...
  boundary@data$job_availability = 0.0
  boundary@data$job_availability <- with(boundary@data, total.x/total.y)
  ...
  publishedinfo = utils.publishSP2GeoServerWithMultiStyles(spobj=boundary, 
                                                               attrname_vec=c("JobAvailability"),
